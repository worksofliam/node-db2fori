...
module.exports = class DatabaseConnection {
  ...
  async sendSQLRPBRequest(attributes, doReply, rpbID, rpbRequestType) {
    ...
    const data = DatabaseRequestAttributes.getDataBuffer(attributes);

    length += data.length;

    await this.writeHeader(length, rpbRequestType);
    await this.writeTemplate(data.parms, (doReply ? OperationalResultBitmap.SEND_REPLY_IMMED : 0x00000000), 0, rpbID);

    await this.socket.writeBuffer(data.buffer);
  }
   ...
   * @param {DatabaseRequestAttributes} attributes 
   */
  async prepareAndDescribe(attributes) {
    this.socket.flush();

    await this.sendPrepareAndDescribeRequest(attributes);
    
    await this.socket.wait();

    await this.parseReply("prepareAndDescribe");
  }

  /**
   * Used to request metadata for a select statement
   * @param {DatabaseRequestAttributes} attributes 
   */
  async sendPrepareAndDescribeRequest(attributes) {
    var length = 40;
    // To be on the safe side, default to requesting parameter marker info.
    const hasParameterMarkers = true;

    const data = DatabaseRequestAttributes.getDataBuffer(attributes);

    length += data.length;

    await this.writeHeader(length, 0x1803);
    var template = OperationalResultBitmap.SEND_REPLY_IMMED | OperationalResultBitmap.DATA_FORMAT;

    if (attributes !== undefined && attributes.sqlStatementType === StatementType.CALL) {
    ...
    await this.writeTemplate(data.parms, template);
    await this.socket.writeBuffer(data.buffer);
  }
   ...
   */
  async writeTemplate(parameterCount, orsBitmap = OperationalResultBitmap.SEND_REPLY_IMMED, pmHandle = 0, rpbHandle = this.currentRPB) {
