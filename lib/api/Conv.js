...
module.exports = class Conv {
  static EBCDICBufferToString(buffer, offset, length) {
    var result = '';
    for (var i = 0; i < length; i++) {
      result += String.fromCharCode(CONV_FROM_37[buffer[offset+i]]);
    }
   ...
   * @returns {Buffer}
   */
  static StringToEBCDICBuffer(string) {
    var buffer = Buffer.alloc(string.length);

    var charCode, ebcdicChar;
    for (var i = 0; i < string.length; i++) {
      charCode = string.charCodeAt(i);
      ebcdicChar = COVV_TO_37[string.charCodeAt(i)];
      buffer.writeUInt8(ebcdicChar, i);
    }
  ...
  /**
   * 
   * @param {Buffer} stringBuffer EBCDIC string buffer
   * @returns {Number}
   */
  static EBCDICStringLength(stringBuffer) {
    for (var i = 0; i < stringBuffer.length; i++) {
      if (stringBuffer[i] === 0x40 || stringBuffer[i] === 0) return i;
    }
   ...
   * @param {number} scale 
   */
  static packedDecimalToString(data, offset, digits, scale) {
    if (digits % 2 === 0) digits += 1;

    const length = Math.floor(digits / 2) + 1;

    const sign = data[offset+length-1] & 0x0D;
    const isNegative = sign === 0x0B || sign === 0x0D;

    var buffer = Buffer.alloc(digits + (scale > 0 ? 1 : 0) + (isNegative ? 1 : 0));

    var count = 0;

    if (isNegative) buffer[count++] = '-';

    const doHigh = (digits % 2) !== 0;
    const digitsBeforeDecimal = digits - scale;
    var foundNonZero = false;
    var nibble;

    for (var i = 0; i < digitsBeforeDecimal; ++i) {
      nibble = (doHigh ? (data[offset] >> 4) : data[offset]) & 0x0F;
    ...
    for (var i = digitsBeforeDecimal; i < digits; ++i) {
      nibble = (doHigh ? (data[offset] >> 4) : data[offset]) & 0x0F;
      buffer[count++] = NUM[nibble];
