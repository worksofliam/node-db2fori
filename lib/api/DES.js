...
module.exports = class DES {
  ...
  static encrypt(key, data) {
    let e1 = Buffer.alloc(65);
    e1.fill(0);
    let e2 = Buffer.alloc(65);
    e2.fill(0);
    ...
    // encryption method
    let preout = Buffer.alloc(65); // preoutput block
    preout.fill(0);

    // temp key gen workspace
    let Cn = Buffer.alloc(58);
    Cn.fill(0);
    ...
    let key1 = Buffer.alloc(49); // 48 bit key 1 to key 16
    key1.fill(0);
    ...
    let Ln = Buffer.alloc(33);
    Ln.fill(0);
    // ditto
    let Rn = Buffer.alloc(33);
    Rn.fill(0);
  ...
  static cipher(key, Ln, Rn) {
    let temp1 = Buffer.alloc(49); // Rn run through E
    temp1.fill(0);
    let temp2 = Buffer.alloc(49); // temp1 XORed with key
    temp2.fill(0);
    let temp3 = Buffer.alloc(33); // temp2 run through S boxes
    temp3.fill(0);
    let fkn = Buffer.alloc(33); //  f(k,n)
    fkn.fill(0);
    ...
    let temp = Buffer.alloc(33); // storage for Ln during cipher function
    temp.fill(0);
    Rn.copy(temp, 1, 1, 33);
    for (let n = 1; n <= 32; n++) {
